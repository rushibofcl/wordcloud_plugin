...
2.  Develop your plugin code in `src/dp_wordcloud_plugin`

    The `src/dp_wordcloud_plugin/Plugin.tsx` is the main plugin file where you have to register all
    your custom content. However, you can create as many new files as you want under the `src/dp_wordcloud_plugin`
...
5.  Add plugin to one or more workspaces: `npm run add-plugin -- <url>`
    ...
    Run the `npm run add-plugin -- "https://your.hosting/pluginDirOfYourChoice/dp_wordcloud_plugin.mjs"` to
...
6.  Use plugin on a dashboard: `npm run link-plugin -- <plugin-object-id>`
    ...
    If your plugin supports parameterization (see [src/dp_wordcloud_plugin](./src/dp_wordcloud_plugin/Plugin.tsx)) and
...
This project is setup so that all your custom code must be self-contained in the [src/dp_wordcloud_plugin](./src/dp_wordcloud_plugin) directory.

The [src/dp_wordcloud_plugin\_engine](./src/dp_wordcloud_plugin_engine) and [src/dp_wordcloud_plugin\_entry](./src/dp_wordcloud_plugin_entry) directories contain essential plugin boilerplate.
...
You can edit the compatibility property in `src/dp_wordcloud_plugin\_entry/index`.
